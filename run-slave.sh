...
start_slave_cmd='export MYSQL_PWD='$ADMIN_PASS'; mysql -u '$ADMIN_USER' -e "'
start_slave_cmd+="$start_slave_stmt"
start_slave_cmd+='"'
docker exec $DB_CONTAINER_NAME sh -c "$start_slave_cmd"

echo "get slave status"
docker exec $DB_CONTAINER_NAME sh -c "export MYSQL_PWD='$ADMIN_PASS'; mysql -u '$ADMIN_USER' -e 'SHOW SLAVE STATUS \G'"
